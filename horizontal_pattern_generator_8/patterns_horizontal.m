...
phase_shifts = [-pi/2, 2*pi/3-pi/2, -2*pi/3-pi/2];
...
for i = 1:length(phase_shifts)
    ...
    fringe_pattern = sin(2 * pi * frequency * Y + phase_shift); % X와 Y를 교환
...
% Access the normalized fringe patterns using fringe_patterns_normalized{1}, fringe_patterns_normalized{2}, etc.
